...
-------------------------------------------- MODIFIED FORM THE EXMAPLE AS: ----------------------------------------------------------------

This code is almost exactly the same as the example code for BLE Central exmaple. I had added a few `ESP_LOGX` statements to the code 
for debugging purposes, which have been commented out now. This code also has commented the line no. 505, which prints the addr fo device 
its trying to connect to. 

It uses the default connection params when trying to connect to an advertiser. 
